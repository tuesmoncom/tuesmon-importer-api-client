...
class ProjectTuesmonSpec extends TuesmonSpecBase {

    void 'get a project'() {
        given: 'an existing project'
            def project = tuesmonClient.createProject(buildBasicProject())
            assert tuesmonClient.getProjects().size() > 0

        when: 'tying to get the project'
            def sameProject = tuesmonClient.getProjectById(project.id)
    ...
    void 'get the list of projects'() {
        ...
        when: 'getting the list of projects'
            def projects = tuesmonClient.getProjects()
